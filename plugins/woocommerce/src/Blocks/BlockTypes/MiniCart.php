...
class MiniCart extends AbstractBlock {
	...
	protected function render_experimental_iapi_mini_cart( $attributes, $content, $block ) {
		...
		if ( $cart ) {
			...
			// Per-user values are only computed when hydration is allowed. When
			// CDN caching is on, we emit neutral defaults here and let the
			// client-side Interactivity store populate real values post-load.
